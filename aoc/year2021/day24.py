...
class ALU:
    ...
    def process(self, instruction, var, *args):
        if args:
            ...
            arg = None
        match instruction:
            case 'inp': self.vars[var] = self.inputs.popleft()
            case 'add': self.vars[var] += arg
            case 'mul': self.vars[var] *= arg
            case 'div': self.vars[var] //= arg
            case 'mod': self.vars[var] %= arg
            case 'eql': self.vars[var] = int(self.vars[var] == arg)
