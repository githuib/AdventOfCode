...
from aoc.geo2d import P2, intersect_lines_2
from aoc.geo3d import P3D, Mat3D
...
def check(h1: Trajectory, h2: Trajectory, check_range: P2) -> bool:
...
class Problem1(_Problem):
    ...
    def solution(self) -> int:
        ...
        return sum(check(h1, h2, check_range) for h1, h2 in self.hailstone_pairs)
...
class Problem2(_Problem):
    ...
    def solution(self) -> int:
        """
        Credits to https://www.reddit.com/user/TheZigerionScammer/ for this approach:
        https://www.reddit.com/r/adventofcode/comments/18pnycy/comment/keqf8uq/
        """
        velo_range = Mat3D(v for _, v in self.parsed_input).size
        ...
        if not all(len(v) == 1 for v in potential_v):
            ...
            logging.warning('Multiple possible velocity matches found:\nx: %s\ny: %s\nz: %s', *potential_v)

        # While other stations' listeners are at school... we're shoplifting! V-Rock - the home of the vulture.
