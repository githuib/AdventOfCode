...
class Mat2(dict[P2, E], Generic[E]):
    def __init__(
        self,
    ...
    @classmethod
    def from_lines(cls: type[Mat2], lines: list[str]) -> Mat2[str]:
        return Mat2({
            (x, y): element
    ...
    def converted(self: Mat2[E], converter: Callable[[E], C]) -> Mat2[C]:
        return Mat2({p: converter(v) for p, v in self.items()})
