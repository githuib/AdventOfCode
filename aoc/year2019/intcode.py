...
from aoc.problems import MultiLineProblem, T
...
class IntcodeComputer:
    ...
    @classmethod
    def from_str(cls, input_lines: list[str]) -> IntcodeComputer:
        return cls(int(c) for line in input_lines for c in line.split(','))
    ...
    def _read(self, address: int, param_mode: ParamMode) -> int:
        ...
        if address < 0:
            raise IndexError('Cannot read from negative address.')
    ...
    def _write(self, address: int, value: int, param_mode: ParamMode) -> None:
        if param_mode == ParamMode.IMMEDIATE:
            raise ValueError('Invalid param mode for writing.')
        if param_mode == ParamMode.RELATIVE:
            address = self._relative_base + address

        if address < 0:
            raise IndexError('Cannot write to negative address.')
...
class IntcodeProblem(MultiLineProblem[T], ABC, Generic[T]):
    computer: IntcodeComputer

    def process_input(self) -> None:
        super().process_input()
        self.computer = IntcodeComputer.from_str(self.lines)
